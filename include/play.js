...
const { canModifyQueue, STAY_TIME, LOCALE } = require("../util/ayam");
const i18n = require("i18n");
i18n.setLocale(LOCALE);

module.exports = {
  async play(song, message) {
    const { SOUNDCLOUD_CLIENT_ID } = require("../util/ayam");
    ...
    collector.on("collect", (reaction, user) => {
      ...
      switch (reaction.emoji.name) {
        ...
        case "🔇":
          ...
          if (queue.volume <= 0) {
            queue.volume = 100;
            queue.connection.dispatcher.setVolumeLogarithmic(100 / 200);
