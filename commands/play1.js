...
const { YOUTUBE_API_KEY, LOCALE } = require("../util/ayam");
...
module.exports = {
  ...
  async execute(message, args) {
    ...
    const src = args.join(" ");
    const search = src.concat(" Audio");
    console.log(message.member.voice + " requested " + search);
    ...
    try {
      ...
      const response = await message.channel.awaitMessages(filter, { max: 1, time: 150000, errors: ["time"] });
      ...
      if (reply.includes(",")) {
        ...
        for (let song of songs) {
          ...
          message.client.commands.get("pl").execute(message, [choice]);}
          else{
            resultsMessage.delete().catch(console.error);
            message.reply("**Pencarian dibatalkan, silahkan cari ulang lagi**").catch(console.error);
          }
      }
